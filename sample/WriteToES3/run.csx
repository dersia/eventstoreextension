...
public static async Task<IActionResult> Run(HttpRequest req, IAsyncCollector<byte[]> data, Microsoft.Extensions.Logging.ILogger log)
{
    string requestBody = await new StreamReader(req.Body).ReadToEndAsync();

    await data.AddAsync(System.Text.Encoding.UTF8.GetBytes(requestBody));
